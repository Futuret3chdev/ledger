...
export function normalizeBill(input) {
  const bill = {
    ...
    subcontractorId: text(input?.subcontractorId, 80),
...
export function normalizeBook(input) {
  ...
  const subcontractors = namedList(input.subcontractors, 'Subcontractor', LIMITS.subcontractors, (row) => ({
    abn: text(row?.abn, 14),
    work: text(row?.work, 80),
  }));
  ...
  const subIds = new Set(subcontractors.map((row) => row.id));
  for (const bill of bills) {
    ...
    if (bill.subcontractorId && !subIds.has(bill.subcontractorId)) throw new ValidationError('A bill points at a missing subcontractor');
  }
...
function normalizeBank(raw) {
  ...
  if (connections.length > 40) throw new ValidationError('Too many bank connections');
  return {
    provider: raw.provider === 'basiq' ? 'basiq' : '',
    userId: text(raw.userId, 80),
    email: text(raw.email, 120),
    mobile: text(raw.mobile, 20),
    lastSyncAt: typeof raw.lastSyncAt === 'string' && !Number.isNaN(Date.parse(raw.lastSyncAt)) ? raw.lastSyncAt : null,
    connections: connections.map((row) => ({
      id: text(row?.id, 80),
      institution: text(row?.institution, 80),
      status: text(row?.status, 40),
      accounts: Array.isArray(row?.accounts)
        ? row.accounts.slice(0, 80).map((acc) => ({
            id: text(acc?.id, 80),
            name: text(acc?.name, 80) || 'Account',
            masked: text(acc?.masked, 24),
            balanceCents: Number.isInteger(acc?.balanceCents) ? acc.balanceCents : 0,
          }))
        : [],
    })),
  };
...
function normalizeBankAccounts(raw) {
  ...
  if (list.length > LIMITS.bankAccounts) throw new ValidationError('Too many bank accounts');
  const types = new Set(['everyday', 'credit_card', 'loan', 'term_deposit', 'other']);
  const ids = new Set();
  return list.map((row) => {
    const id = idOf(row?.id, 'Bank account');
    if (ids.has(id)) throw new ValidationError('Two bank accounts share an id');
    ids.add(id);
    const accountType = types.has(row?.accountType) ? row.accountType : 'everyday';
    const bsb = text(row?.bsb, 7).replace(/\D/g, '');
    const number = text(row?.number, 20).replace(/\D/g, '');
    return {
      id,
      name: mustText(row?.name, 80, 'Account name'),
      institution: mustText(row?.institution, 80, 'Bank'),
      institutionId: text(row?.institutionId, 40),
      accountType,
      currency: text(row?.currency, 8) || 'AUD',
      bsb: bsb.length === 6 ? bsb : '',
      number,
      feed: row?.feed === 'basiq' || row?.feed === 'import' ? row.feed : 'none',
    };
  });
}
