...
export async function handle(req, res) {
  ...
  try {
    if (pathname === '/api/health' && req.method === 'GET') {
      return send(res, 200, { ok: true, name: 'Quill', by: 'Futuret3ch and MemeTorrent' });
    }
    if (pathname === '/api/session' && req.method === 'GET') {
      if (!process.env.LEDGER_ACCESS_CODE || !process.env.LEDGER_SESSION_SECRET) {
        return send(res, 503, { error: 'Quill access is not configured' });
      }
    ...
    if (pathname === '/api/feed' && req.method === 'GET') {
      ...
      if (presented) {
        ...
        return send(res, 200, { ok: true, name: 'Quill', feed: true });
      }
      if (!sessionOk(req)) return send(res, 401, { error: 'Locked' });
      return send(res, 200, { ok: true, name: 'Quill', feed: feedTokenConfigured() });
    }
