...
export async function applyBook(mutator) {
  const mode = storageMode();
  if (mode === 'missing') {
    const err = new Error('Storage is not connected');
    err.code = 'NO_STORAGE';
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    const incoming = mutator(current.book);
    const next = {
      ...incoming,
      version: current.book.version + 1,
      updatedAt: new Date().toISOString(),
...
export async function loadPractice() {
  ...
  return {
    ...base,
    ...doc,
    keepers: Array.isArray(doc.keepers) ? doc.keepers : [],
    asks: Array.isArray(doc.asks) ? doc.asks : [],
    roll: Array.isArray(doc.roll) ? doc.roll : [],
